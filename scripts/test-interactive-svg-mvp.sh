...
if [[ $REQUIRES_PREBUILT -eq 1 && ! -f "$ROOT_DIR/test-fixtures/InteractiveSvgMvp/app.aibc1" ]]; then
  echo "interactive svg run-mode checks skipped: runtime requires prebuilt /test-fixtures/InteractiveSvgMvp/app.aibc1"
  exit 0
fi

set +e
out_snapshot="$("$AIVECTRA" run "$ROOT_DIR/test-fixtures/InteractiveSvgMvp/" snapshot 2>&1)"
rc_snapshot=$?
set -e
...
set +e
out_replay="$("$AIVECTRA" run "$ROOT_DIR/test-fixtures/InteractiveSvgMvp/" replay 2>&1)"
rc_replay=$?
set -e
